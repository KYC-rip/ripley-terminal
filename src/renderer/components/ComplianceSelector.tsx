...
export function ComplianceSelector({ 
  ...
  const renderRow = (
    ...
    // ✅ 改动1: 响应式 Padding (py-3 mobile, py-2 desktop) 方便手指点击
    // ✅ 改动2: 响应式 Gap (gap-1 mobile, gap-2 desktop) 防止小屏换行
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
           ...
           <button onClick={() => onSelect('STANDARD')} disabled={disabled} className={`${baseBtn} ${currentLevel === 'STANDARD' ? activeStyles.STANDARD : inactiveBtn}`}>
             ...
             <span>STD</span> {/* 小屏可能缩写? 不，STANDARD还能放下 */} 
             <span className="hidden xs:inline">ARD</span> {/* 可选：极小屏隐藏后缀，或者干脆全显，flex-1通常能搞定 */}
           </button>
