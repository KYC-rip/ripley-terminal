...
export function AuthView({ onUnlock, isInitialSetup, identities, activeId, onSwitchIdentity, onCreateIdentity, logs = [] }: AuthViewProps) {
  ...
  const handleUnlockSubmit = async (e: React.FormEvent) => {
    ...
    // Validation for new setup or restore
    ...
    setTimeout(async () => {
      try {
        ...
        // If we are in LABEL/MODE/RESTORE/NEW_PASSWORD, we are creating/restoring.
        // If we are in AUTH, we are unlocking existing.
        const isCreating = step !== 'AUTH';
        await onUnlock(
          password, 
          step === 'RESTORE' ? restoreSeed : undefined, 
          height,
          isCreating ? newName : undefined
        );
      } catch (err: any) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-xmr-base text-xmr-green font-mono p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md space-y-8 relative z-10 animate-in fade-in zoom-in-95 duration-500">
        <div className="text-center space-y-4">
          ...
          <h1 className="text-3xl font-black italic uppercase text-xmr-green">
            {step === 'RESTORE' ? 'Identity_Recovery' : step === 'NEW_PASSWORD' ? 'Initialize_Security' : 'Vault_Authorization'}
          </h1>
          <div className="flex items-center justify-center gap-2 text-[10px] text-xmr-dim uppercase tracking-[0.2em]">
             <Users size={12} />
             <span>Target: <span className="text-xmr-green font-black">{newName || activeIdentity?.name || 'PRIMARY'}</span></span>
          </div>
        </div>
          ...
          <form onSubmit={handleUnlockSubmit} className="space-y-6">
            ...
            {step === 'RESTORE' && (
              <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-xmr-dim uppercase ml-1 flex justify-between">
                    <span>Mnemonic_Seed (25 words)</span>
                    <span className="text-xmr-accent font-black">STANDARD_ONLY</span>
                  </label>
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-black text-xmr-dim uppercase ml-1">
                {step !== 'AUTH' ? 'Set_Vault_Password' : 'Enter_Vault_Password'}
              </label>
            ...
            {error && (
              <div className="p-3 bg-red-900/20 border border-red-600/50 text-red-500 text-[10px] font-black uppercase flex items-center gap-2 animate-shake">
                <Skull size={14} /> {error}
              </div>
            )}
            ...
            {isProcessing && logs.length > 0 && (
              <div className="p-3 bg-xmr-green/5 border border-xmr-green/10 rounded-sm space-y-1 overflow-hidden">
                 {logs.slice(0, 3).map((log, i) => (
                   <p key={i} className={`text-[8px] uppercase truncate font-black ${i === 0 ? 'text-xmr-green' : 'text-xmr-dim opacity-60'}`}>{'>'} {log}</p>
                 ))}
              </div>
            )}
        ...
        <div className="text-center space-y-4">
          ...
          {step === 'AUTH' && !isInitialSetup && (
            <button onClick={async () => { if (confirm("🚨 WARNING: TOTAL PURGE. This will delete BOTH the local wallet file AND the cached seed. Continue?")) { await (window as any).api.setConfig(`master_seed_${activeId}`, null); await (window as any).api.setConfig(`last_sync_height_${activeId}`, null); await (window as any).api.writeWalletFile({ filename: activeId, buffer: [] }); location.reload(); } }} className="text-[8px] text-red-900 hover:text-red-500 uppercase font-black underline decoration-dotted underline-offset-4 transition-colors cursor-pointer">[ Nuclear_Identity_Purge ]</button>
          )}
        </div>
      </div>
    </div>
