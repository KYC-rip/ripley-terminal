...
export function SettingsView() {
  ...
  useEffect(() => {
    (window as any).api.getConfig('custom_daemon').then((v: string) => setDaemonUrl(v || ''));
  ...
  const handleSave = async () => {
    ...
    await (window as any).api.setConfig('custom_daemon', daemonUrl);
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 space-y-8 animate-in fade-in slide-in-from-right-4 duration-500 font-mono">
      ...
      <div className="grid grid-cols-1 gap-6">
        <section className="space-y-4">
          ...
          <Card className="p-6 bg-black/40 border-[#004d13]/40 space-y-6">
            <div className="space-y-2">
              <label className="text-[9px] font-bold text-xmr-dim uppercase">Custom_Monero_Remote_Node</label>
              <input 
                type="text" 
                placeholder="https://your-node.com:18081"
                value={daemonUrl}
                onChange={(e) => setDaemonUrl(e.target.value)}
                className="w-full bg-black border border-[#004d13] p-3 text-[10px] text-[#00ff41] focus:border-[#00ff41] outline-none"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center justify-between p-4 bg-[#00ff41]/5 border border-[#00ff41]/20 rounded-sm">
                <div className="space-y-1">
                  ...
                  <p className="text-[8px] opacity-50 uppercase font-bold">Darknet_Tunnel</p>
                </div>
              ...
              <div className={`flex items-center justify-between p-4 border rounded-sm transition-all ${isStagenet ? 'bg-orange-500/10 border-orange-500/40' : 'bg-white/5 border-white/10 opacity-60'}`}>
                <div className="space-y-1">
                  ...
                  <p className="text-[8px] opacity-50 uppercase font-bold tracking-tighter">Testing_Assets</p>
                </div>
        ...
        <section className="space-y-4">
          ...
          <Card className="p-6 bg-black/40 border-[#004d13]/40 flex items-center justify-between">
            <div className="space-y-1"><span className="text-[10px] font-black uppercase">Visual_Scanlines</span><p className="text-[8px] opacity-50 uppercase font-bold">CRT_Effect_Overlay</p></div>
