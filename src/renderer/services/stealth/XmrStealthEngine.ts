...
import { StealthStep, type StealthConfig, type StealthOrder, type StealthLogger, type IncomingTxStatus } from './types';

export class XmrStealthEngine {
  ...
  public async init(rpcUrl: string, password: string = "stealth_session", mnemonic?: string, subaddressIndex?: number, overrideHeight?: number, onHeightUpdate?: (h: number) => void, isStagenet: boolean = false, identityId: string = 'primary') {
    ...
    try {
      ...
      // 🔥 FIX: Provide a robust mock FS to satisfy monero-ts internal checks in Electron renderer.
      // This prevents 'promises of null' errors during getFs() fallback.
      const mockFs = {
        promises: {
          readFile: async () => { throw new Error("FS_MOCK_READ_DISABLED"); },
          writeFile: async () => { },
          mkdir: async () => { },
          unlink: async () => { },
          stat: async () => { throw new Error("FS_MOCK_STAT_DISABLED"); }
        }
      };
      ...
      const walletConfig: Partial<MoneroWalletConfig> = {
        ...
        password: password,
        server: { uri: rpcUrl },
        fs: mockFs, // Injects our mock
        proxyToWorker: false // Run in main renderer thread to avoid serialization issues with mockFs
      };
      ...
      if (savedKeysData) {
        ...
        if (!targetSeed) {
          ...
          const tempWallet = await lib.createWalletFull({ networkType, password: "temp", fs: mockFs, proxyToWorker: false });
  ...
  public async startSyncInBackground(onHeightUpdate?: (h: number) => void) {
    ...
    try {
      ...
      this.logger(`⚠️ Background Sync Suspended: ${e.message}`, 'warn');
    }
  }
