...
import { Shield, Zap, Activity, Lock, Ghost, Database, Plus, RefreshCw, Send, Download, Copy, AlertTriangle, Check, ShieldAlert, Skull, Trash2, Settings, ArrowUpRight, ArrowDownLeft } from 'lucide-react';
...
function MainApp() {
  ...
  if (isInitializing) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#050505] text-[#00ff41] font-mono p-10">
        <style>{`
  ...
  const renderVault = () => (
    <div className="max-w-5xl mx-auto space-y-6 py-4 animate-in fade-in zoom-in-95 duration-300 font-black">
      <div className="flex justify-between items-end border-b border-[#004d13]/30 pb-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={refresh} className="px-3 py-1.5 border border-[#004d13] text-[9px] font-black hover:bg-[#00ff41]/10 transition-all flex items-center gap-2 cursor-pointer uppercase"><RefreshCw size={10} className={status === 'syncing' ? 'animate-spin' : ''} /> Sync_Ledger</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-mono">
        <div className="lg:col-span-2 p-6 border border-[#004d13] bg-[#00ff41]/5 rounded-sm flex flex-col justify-between h-56">
          <div><span className="text-[9px] text-xmr-dim uppercase font-bold tracking-[0.2em]">Liquidity</span><div className="text-5xl font-black mt-2 flex items-baseline gap-3 text-white">{balance.total} <span className="text-xl text-xmr-dim uppercase">XMR</span></div></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="p-6 flex-grow min-h-[300px]">
            {activeTab === 'receive' ? (
              ...
              <div className="space-y-4">
                <div className="space-y-1"><div className="flex justify-between items-center"><label className="text-[9px] font-black text-xmr-dim uppercase">Target_Addr</label>{isBanned && <span className="text-[8px] text-red-500 font-bold animate-pulse flex items-center gap-1 uppercase">Intercepted</span>}</div><input type="text" placeholder="4... / 8..." value={destAddr} onChange={(e) => setDestAddr(e.target.value)} className={`w-full bg-black border p-3 text-[10px] text-white focus:border-[#00ff41] outline-none transition-colors font-black ${isBanned ? 'border-red-600' : 'border-[#004d13]'}`} /></div>
                <div className="space-y-1"><label className="text-[9px] font-black text-xmr-dim uppercase">Amount (XMR)</label><div className="relative"><input type="number" placeholder="0.00" value={sendAmount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-black border border-[#004d13] p-3 text-lg font-black text-[#00ff41] focus:border-[#00ff41] outline-none" /><button className="absolute right-3 top-3 text-[9px] font-black text-xmr-dim hover:text-white uppercase cursor-pointer font-mono">[ MAX ]</button></div></div>
        ...
        {/* --- NEW HISTORY / LEDGER AREA --- */}
        <div className="border border-[#004d13] bg-black/20 rounded-sm overflow-hidden flex flex-col h-[350px]">
          <div className="px-4 py-3 border-b border-[#004d13]/20 bg-black/40 text-[9px] font-black uppercase tracking-widest flex justify-between items-center">
            <span>Identity_Ledger</span>
            <span className="text-[8px] opacity-40 uppercase font-black">{txs.length} Records</span>
          </div>
          <div className="flex-grow overflow-y-auto font-mono text-[9px]">
            {txs.length > 0 ? (
              <div className="divide-y divide-[#004d13]/10">
                {txs.map((tx, i) => (
                  <div key={i} className="p-3 hover:bg-[#00ff41]/5 transition-colors group">
                    <div className="flex justify-between mb-1">
                      <div className="flex items-center gap-2">
                        {tx.isIncoming ? (
                          <ArrowDownLeft size={12} className="text-[#00ff41]" />
                        ) : (
                          <ArrowUpRight size={12} className="text-[#ff6600]" />
                        )}
                        <span className={`font-black ${tx.isIncoming ? 'text-[#00ff41]' : 'text-[#ff6600]'}`}>
                    ...
                    <div className="flex justify-between items-center">
                      <code className="opacity-20 group-hover:opacity-40 transition-opacity truncate max-w-[180px]">
                        {tx.id}
                      </code>
                      <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-xs ${tx.confirmations >= 10 ? 'bg-[#00ff41]/10 text-[#00ff41]' : 'bg-white/5 text-white/40'}`}>
                        {tx.confirmations >= 10 ? 'CONFIRMED' : `PENDING (${tx.confirmations}/10)`}
                      </span>
                    </div>
                  </div>
                ))}
              ...
              <div className="h-full flex items-center justify-center italic opacity-20 uppercase tracking-widest">
                No_Records_Logged
              </div>
            )}
          </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#050505] text-[#00ff41] font-mono relative overflow-hidden select-none">
      ...
      <header className="h-14 flex items-center px-6 border-b border-[#004d13]/40 bg-black/60 backdrop-blur-md justify-between z-50" style={{ WebkitAppRegion: 'drag' } as any}>
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => setView('home')}><Shield size={18} className="text-[#00ff41] animate-pulse" /><div className="flex flex-col"><span className="text-xs font-black tracking-[0.4em]">GHOST_TERMINAL</span><span className="text-[8px] opacity-40 font-bold tracking-widest uppercase tracking-[0.2em]">Secure_Uplink</span></div></div>
